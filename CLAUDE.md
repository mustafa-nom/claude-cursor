...
| `CompanionPanelSoundFeedback.swift` | Panel-only UI sounds via `AVAudioPlayer`: `enter.mp3` when the user changes feature toggles or model in `CompanionPanelView`, `eshop.mp3` when the panel auto-opens on launch (`showPanelOnLaunch`) and on app quit (`applicationWillTerminate`). |
...
| `PatternDatabase.swift` | SQLite3-backed persistence for lesson progress, session metadata, tutor nudge rate limiting, confidence scores with time-based decay, interaction outcome tracking, `computer_use_runs` analytics, and the chat sidebar's `chat_session_segments` + `chat_session_segment_turns` tables (with `PRAGMA foreign_keys = ON` so the CASCADE delete on segment turns actually fires). Uses built-in libsqlite3 (no external dependency). |
| `ChatWindowController.swift` | Floating `NSPanel` hosting the 640pt × 520pt chat split view. Left pane: `ChatSidebarView` with "+ New Chat" + search + date-bucketed app folders. Right pane: `ChatLiveTranscriptView` (live transcript + text input, same voice + screenshot + wiki pipeline) OR `ReadOnlySegmentTranscriptView` (past segment) depending on the sidebar selection. Toggled by the "Show chat" menu bar toggle. |
| `ChatSidebarView.swift` | SwiftUI left-pane sidebar for the chat window. Renders a 220pt-wide Granola-style tree: pinned "Current" row, date-bucketed sections (TODAY / YESTERDAY / THIS WEEK / LAST WEEK / month labels), app folders with `CachedAppIconView` icons, browser-tool sub-folders with hash-tinted initials avatars, and an "Older sessions — N not yet indexed" footer that triggers additional backfill. All state delegated to `ChatSidebarModel`. |
| `ChatSidebarModel.swift` | `@MainActor ObservableObject` backing the chat sidebar. Fetches from `PatternDatabase`, subscribes to `.chatSessionSegmentsDidChange`, builds the date-section → app-folder → (optional browser-tool sub-folder) → segment-row tree, swaps to a flat search-result list when the query box is non-empty, persists expand/collapse state per folder key in UserDefaults, and exposes a `requestBackfillOfAdditionalOlderSessions()` hook for the footer row. |
| `ChatSessionSegmenter.swift` | End-of-session splitter. Re-parses the session log from disk via `ChatSessionLogParser` (avoids `ObserverAgent`'s 50-turn in-memory cap), groups consecutive turns by `(bundleIdentifier, browserToolName)`, applies a 3-minute sandwich-merge rule to absorb single-turn context-switch interludes, inserts rows with heuristic titles (first user utterance trimmed to 60 chars), then fires a single batched Claude call per session asking for all K titles as JSON and updates the rows in place. Posts `.chatSessionSegmentsDidChange` on both insert and rename. The splitter itself is a pure static function so `ChatSessionSegmenterTests` can exercise the boundary rules without a DB. |
| `ChatSessionLogParser.swift` | Shared parser for `raw/sessions/session_*.md`. Tolerates two on-disk formats: legacy 1-field brackets (`[AppName]`) and the new 4-field brackets (`[AppName\|bundleID\|hostname-or-dash\|tool-or-dash]`). Used by both `ChatSessionSegmenter` (on session end) and `ChatSessionBackfillRunner` (on first launch). |
| `ChatSessionBackfillRunner.swift` | One-shot pass that retro-files existing `raw/sessions/session_*.md` logs into the new chat segments table on first launch post-ship. Gated by the `ClaudeCursor.chatSegmentsBackfillVersion` UserDefaults key (first run caps at 50 newest). Idempotent: a per-session existence check against `PatternDatabase.distinctParentSessionIDs()` prevents duplicate rows if a partial run was interrupted. `backfillAdditionalUnprocessedSessions(...)` drives the sidebar's "Older sessions" footer; sequential throttle keeps batched Claude rename calls from stacking. |
| `BrowserTabURLExtractor.swift` | Resolves active-tab URL for Chromium browsers (Chrome / Arc / Brave / Edge) by BFS-walking the AX tree of the focused window for a `kAXTextFieldRole` + `AXAddressField` subrole element. Hard 150ms timeout via `DispatchQueue.global().async` + `DispatchSemaphore.wait(timeout:)` so a wedged browser can't block the main actor. Returns only hostname + derived tool name to the segmenter — full URL stays in memory on `ChatTurnRecord.browserTabURLInMemoryOnly` and is never persisted (avoids leaking session tokens / search queries into the wiki). Safari is intentionally unsupported in v1. |
| `ReadOnlySegmentTranscriptView.swift` | Right-pane view when the sidebar has a past segment selected. Queries `PatternDatabase.listSegmentTurns(segmentID:)` once on appear, renders turns via the shared `ChatMessageBubble`, and exposes a "Back to current chat" ghost button that asks the parent to flip `currentSelection = .liveCurrent`. No text input. |
| `AppIconCache.swift` | Memoized `NSWorkspace.icon(forBundleIdentifier:)` lookups so the chat sidebar doesn't hit disk on every scroll. Exposes a shared instance + a `CachedAppIconView` SwiftUI wrapper that falls back to an `app.fill` SF Symbol when the bundle ID is empty (legacy sessions) or unresolvable. |
...
| `ObserverAgent.swift` | Session-scoped observer. Writes append-only `raw/sessions/session_<id>.md` logs for every user↔Claude turn (PII-stripped via `PIIStripper`), tracks frontmost apps + turn count, and on session end hands the log to `SessionCompressor` for wiki ingestion. Turn headers are emitted in the 4-field bracket format `### <iso> [<appName>\|<bundleID>\|<hostname or "-">\|<tool or "-">] (<mode>)` so the chat session segmenter can group sidebar entries by `(bundleIdentifier, browserToolName)`; legacy 1-field logs remain parseable via `ChatSessionLogParser`. |
...
| `ComputerUseSupport.swift` | Shared Computer Use helpers: aspect-ratio-matched JPEG resize, frontmost-window target display selection, `ComputerUseRunMetrics` (stuck counter plus wall-clock, screenshot count, and the three Anthropic input-token buckets + output tokens for cache-aware telemetry), `ComputerUseRunLogger` which injects `schema_version: 2` into every JSONL event plus `appendRollupSummary` for the one-line-per-run `rollup.log`, and `ComputerUseTypeTextNormalization` so mistaken literal `return` suffixes on `type` actions become a real Return keypress. |
...
| `claude-cursorTests/ChatSessionSegmenterTests.swift` | Swift Testing cases for `ChatSessionSegmenter.splitTurnsIntoSegments`. Covers all-same-bundle (no splits), multi-bundle (clean split), legacy empty-bundleID logs (single segment), same-bundle-different-browser-tool (split by tool), sandwich-merge at tight and wide gaps, start-edge and end-edge singletons (always kept), two adjacent singletons, 50-turn single-bundle session, and the empty-turns no-op. |
