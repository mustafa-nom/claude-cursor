...
# =============================================================================
# release.sh — Automates the full release pipeline for ClaudeCursor
#
...
#   9. Pushes the updated appcast.xml to the GitHub repo (see GITHUB_REPO)
#
# Usage:
...
SCHEME="claude-cursor"
APP_NAME="ClaudeCursor"
...
# Optional: add repo-root `dmg-background.png` for a branded drag-to-Applications layout.
DMG_BACKGROUND="${PROJECT_DIR}/dmg-background.png"

GITHUB_REPO="${GITHUB_REPO:-mustafa-nom/claude-cursor}"
...
if [[ -f "${DMG_BACKGROUND}" ]]; then
    ...
    echo "   (no ${DMG_BACKGROUND##*/} — using default DMG window)"
fi
create-dmg "${CREATE_DMG_ARGS[@]}" "${DMG_PATH}" "${EXPORT_DIR}/${APP_NAME}.app" \
    2>&1 | tail -3
...
gh release create "${TAG}" "${DMG_PATH}" \
    ...
    --notes "ClaudeCursor v${MARKETING_VERSION}" \
    --latest

# ── Step 9: Push appcast.xml to the releases repo ───────────────────────────
# Commits the generated appcast into GITHUB_REPO (default: mustafa-nom/claude-cursor;
# override with env GITHUB_REPO). Clones to a temp dir, copies appcast.xml, pushes.
